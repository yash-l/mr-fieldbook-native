(() => {
  ...
  function intelligenceReasons(doctor){
    const reasons=[],access=doctorAccessForDate(doctor),timing=doctorMeetingStatus(doctor),quality=doctorCompleteness(doctor),opportunity=productOpportunity(doctor),last=latestDoctorVisit(doctor.id,true),notMet=recentNotMetCount(doctor.id);
...
function nearbyResultCard(x){
  ...
  return `<button class="nearby-place-card plain-button" data-nearby-place-id="${esc(x.id)}"><div class="nearby-place-distance">${esc(x.distanceKm.toFixed(2))}<small>km</small></div><div class="nearby-place-copy"><h3>${esc(x.name)}</h3><p>${esc(x.address||'Address unavailable')}</p><small>${esc(timingText)}${hours?` • ${esc(hours)}`:''}</small></div><span class="source-pill ${x.source==='saved'?'':'live'}">${esc(nearbySourceLabel(x.source))}</span></button>`;
}
...
function applyNearbyProviderResults(prefix,ok,json,error,provider='google',cached=false){
  ...
  try{
    ...
    if(quick){out.innerHTML=merged.length?merged.slice(0,12).map(x=>`<button type="button" class="nearby-place-card plain-button" data-quick-nearby-place="${esc(x.id)}"><div class="nearby-place-distance">${esc(x.distanceKm.toFixed(2))}<small>km</small></div><div class="nearby-place-copy"><h3>${esc(x.name)}</h3><p>${esc(x.address||'Address unavailable')}</p><small>${esc((x.doctorIds||[]).length)} saved doctor(s) • ${esc(nearbySourceLabel(x.source))}</small></div></button>`).join(''):empty('No hospital or clinic found within 2 km.');}
...
function chooseNearbyHospital(place){
  ...
  openSheet(place.name,`${place.distanceKm.toFixed(2)} km away • ${place.source==='google'?'Google place':place.source==='osm'?'OpenStreetMap place':'saved master location'}`,`<div class="detail-section"><h4>Hospital details</h4><div class="note-box">${esc(place.address||'Address unavailable')}<br><a href="${mapUrl(place.latitude,place.longitude)}" target="_blank" rel="noopener">Open exact map</a></div></div><div class="detail-section"><h4>Which doctor do you want to meet?</h4><div id="nearbyDoctorResults">${linked.length?linked.map(d=>`<button class="mini-card plain-button" data-nearby-doctor-id="${d.id}"><span class="mini-icon">⚕</span><span class="mini-copy"><h3>${esc(d.name)}</h3><p>${esc([doctorMeetingStatus(d).label,linkedChemist(d)?.name].filter(Boolean).join(' • '))}</p></span></button>`).join(''):empty('No doctor is linked to this hospital yet. Search the accurate doctor below.')}</div><label class="search-box nearby-doctor-search"><span>⌕</span><input id="nearbyDoctorSearch" type="search" placeholder="Search accurate doctor name…"></label><div id="nearbyDoctorSearchResults" class="search-results lookup-results hidden"></div></div><div class="google-attribution">© OpenStreetMap contributors when OSM data is shown • Google attribution applies when Google data is shown.</div>`);
  const selectDoctor=id=>{const d=doctorById(id);if(!d)return;d.hospital=place.name;d.address=place.address||d.address;d.latitude=place.latitude;d.longitude=place.longitude;if(place.source==='google'){d.placeId=place.placeId||d.placeId||'';}else if(place.source==='osm'){d.osmId=place.osmId||d.osmId||'';}d.hospitalOpeningHours=place.openingHours||d.hospitalOpeningHours||[];d.locationSource=place.source==='google'?'Google Places nearby match':place.source==='osm'?'OpenStreetMap Overpass nearby match':'Saved location';d.locationCapturedAt=new Date().toISOString();d.updatedAt=new Date().toISOString();saveState(false);closeSheet();quickMeeting(d.id,'');toast('Hospital name and exact location linked to doctor.');};
...
function orderMiniCard(o){
  ...
  function renderDoctors() {
    ...
    let list=state.doctors.filter(d=>{
      ...
      else if(doctorFilter==='available')matchF=doctorMeetingStatus(d).state==='available';
  ...
  function recordCard(r,type) {
    ...
    return `<article class="record-card"><div class="record-top"><div class="avatar">${esc(initials(r.name))}</div><div class="record-title"><div class="title-line"><h3>${esc(isDoctor?doctorDisplayName(r):r.name)}</h3>${typeTag}</div><p>${esc(subtitle||'Details not added')}</p></div></div>${r.address?`<p class="record-note">${esc(r.address).slice(0,180)}</p>`:''}<div class="tag-row">${todayAvailabilityTag}${timingTag}${productTags}${tags.slice(0,2).map(t=>`<span class="tag">${esc(t)}</span>`).join('')}</div><div class="record-actions ${isDoctor?'doctor-actions-four':'chemist-actions-four'}">${actions}</div></article>`;
  }
