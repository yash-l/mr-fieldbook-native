...
function defaultSchemes() {
  ...
  function doctorGoogleQuery(doctor,place=null){const parts=[doctor?.name,place?.name||doctorHospital(doctor),doctorType(doctor),place?.address||doctor?.address||doctor?.hospitalAddress,inferDoctorArea(doctor),doctor?.hq||state.profile.hq,'Gujarat','India'];return parts.map(cleanGpsQueryPart).filter(Boolean).join(', ');}
  ...
  function doctorLocationVerification(doctor){const hasGps=Boolean(num(doctor?.latitude)&&num(doctor?.longitude)),source=norm(doctor?.locationSource),mode=norm(doctor?.gpsResolutionMode);const googleAuto=Boolean(doctor?.googleCrossCheckedAt||doctor?.placeId&&(source.includes('google')||mode.includes('google'))),visitVerified=source.includes('visitgpsverified')||norm(doctor?.locationVerificationStatus)==='verified';const manual=Boolean(doctor?.googleManualCheckedAt);const verified=hasGps&&(googleAuto||visitVerified||manual);return {hasGps,verified,level:verified?'verified':hasGps?'review':'missing',label:verified?'Google/GPS verified':hasGps?'Google cross-check pending':'GPS missing'};}
  ...
  function openDoctorLocationVerification(id,place=null){const d=doctorById(id);if(!d)return;const lat=num(place?.latitude||d.latitude),lng=num(place?.longitude||d.longitude),query=doctorGoogleQuery(d,place),status=doctorLocationVerification(d),googleUrl=googleDoctorVerifyUrl(d,place);locationVerifyContext={doctorId:id,place:place||null,latitude:lat,longitude:lng,query};openSheet('Verify doctor location','Cross-check doctor + hospital + doctor type before Field AI trusts the pin.',`<div class="verification-card"><span class="verification-badge ${status.level}">${esc(status.label)}</span><h3>${esc(d.name)} <em>${esc(doctorType(d))}</em></h3><p>${esc(place?.name||doctorHospital(d)||'Hospital not linked')}</p><small>${esc(place?.address||d.address||d.hospitalAddress||'Address not entered')}</small></div><div class="note-box"><strong>Google query</strong><br>${esc(query)}</div><div class="button-row"><a class="btn secondary" href="${googleUrl}" target="_blank" rel="noopener">Open Google Maps</a>${hasOptionalGooglePlaces()?'<button id="autoGoogleCrossCheckBtn" class="btn primary" type="button">Auto cross-check</button>':''}</div>${lat&&lng?`<a class="btn secondary full" href="${mapUrl(lat,lng)}" target="_blank" rel="noopener">Open saved/selected pin</a>`:''}<button id="manualGoogleConfirmBtn" class="btn primary full" type="button" ${lat&&lng?'':'disabled'}>I checked Google — location matches ✓</button><div id="googleCrossCheckResults" class="card-list compact-list"></div><small class="muted-line">Free mode uses the official Google Maps search URL; Google does not return place data to the app without Places API. If a Places key is configured, Auto cross-check compares live candidates.</small>`);$('#manualGoogleConfirmBtn')?.addEventListener('click',()=>{const ctx=locationVerifyContext,doc=doctorById(ctx?.doctorId);if(!ctx||!doc||!ctx.latitude||!ctx.longitude)return;if(ctx.place){doc.hospital=clean(ctx.place.name)||doc.hospital;doc.address=clean(ctx.place.address)||doc.address;doc.latitude=ctx.latitude;doc.longitude=ctx.longitude;if(ctx.place.osmId)doc.osmId=ctx.place.osmId;}doc.googleManualCheckedAt=new Date().toISOString();doc.locationVerificationStatus='verified';doc.locationSource='Google Maps manual cross-check: doctor + hospital + type';doc.updatedAt=new Date().toISOString();saveState(false);closeSheet();haptic('success');toast('Location verified. Field AI can use this doctor in the GPS patch.');});$('#autoGoogleCrossCheckBtn')?.addEventListener('click',()=>{const out=$('#googleCrossCheckResults');if(out)out.innerHTML='<div class="notice">Checking Google Places…</div>';window.AndroidBridge?.searchDoctorPlaces?.('location-verify',query);});}
  function crossCheckScore(row,ctx){const d=doctorById(ctx?.doctorId);if(!d)return 0;const hospital=norm(ctx?.place?.name||doctorHospital(d)),candidate=norm(`${row.name||''} ${row.address||''} ${row.primaryType||''}`),type=norm(doctorType(d));let score=0;if(hospital&&candidate.includes(hospital))score+=45;if(type&&candidate.includes(type))score+=10;const baseLat=num(ctx?.latitude||d.latitude),baseLng=num(ctx?.longitude||d.longitude);if(baseLat&&baseLng&&num(row.latitude)&&num(row.longitude)){const km=haversineKm(baseLat,baseLng,num(row.latitude),num(row.longitude));if(km<=.1)score+=50;else if(km<=.3)score+=38;else if(km<=.8)score+=20;else if(km<=2)score+=6;}return score;}
