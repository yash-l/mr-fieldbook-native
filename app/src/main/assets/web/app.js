...
function updateOrderTotal(root){const total=collectOrderItems(root).reduce((n,x)=>n+x.value,0),el=$('[data-order-total]',root);if(el)el.textContent=`₹${total.toLocaleString('en-IN',{maximumFractionDigits:2})}`;}
  ...
  function meetingSummaryHtml(doctor,chemist) {
    ...
    const last=latestPairVisit(doctor.id,chemist?.id||'');const history=[last?`Last: ${prettyDate(last.date)}`:'First recorded call',doctor.nextFollowUp?`Follow-up: ${prettyDate(doctor.nextFollowUp)}`:''].filter(Boolean).join(' • ');return `<div class="meeting-summary"><div><small>DOCTOR / HOSPITAL</small><strong>${esc(doctorDisplayName(doctor))}</strong><p>${esc([doctor.address||doctor.area||'Address pending',history].filter(Boolean).join(' • '))}</p></div><div><small>UNDER CHEMIST</small><strong>${esc(chemist?.name||'Chemist pending')}</strong><p>${esc(chemist?.address||chemist?.area||'Can be completed later')}</p></div><div class="meeting-timing-cell ${esc(timing.state)}"><small>CLINIC ACCESS / MEETING</small><strong>${esc(doctorClinicSystemLabel(doctor))}</strong><p>${esc(access||fullTiming||timing.label)}</p></div>${map?`<a href="${map}" target="_blank" rel="noopener">Open stored map</a>`:''}</div>`;
  }
  ...
  function quickMeeting(doctorId='',chemistId='') {
    ...
    openSheet('Log meeting','Search doctor or hospital; linked details fill automatically.',`
      <form id="meetingForm" class="sheet-form">
        <details class="fast-meeting-tool"><summary>🎙 Voice fill</summary><div class="fast-meeting-tool-body">${voiceControlsHtml('meeting')}</div></details>
        <div class="lookup-label field-block fast-doctor-search"><span class="field-caption">Doctor / hospital</span>
        ...
        <details id="meetingMasterDetails" class="schedule-card master-data-card fast-master-details" ${doctor&&doctorHospital(doctor)&&linkedChemist(doctor)?'':'open'}>
          <summary><span><b>Saved doctor details</b><small>Hospital, chemist & timing • edit only when changed</small></span><span class="fast-master-state">${doctor&&doctorHospital(doctor)&&linkedChemist(doctor)?'Ready':'Complete once'}</span></summary>
          <div class="fast-master-body">
          <div class="form-section-title"><h3>Hospital, pharmacy & doctor timing</h3><p>Saved to doctor master and auto-filled next time.</p></div>
        ...
        <details class="fast-meeting-tool"><summary>⌖ Location verification <small>optional</small></summary><div class="fast-meeting-tool-body"><div class="location-card">
        ...
        <div class="form-section-title fast-result-title"><h3>Meeting result</h3><p>Not met select karne par Machine next suitable meeting and replacement suggest karega.</p></div>
        ...
        <label><span>Follow-up date (optional)</span><input id="meetingFollowUpDate" name="followUpDate" type="date"></label>
        <div class="fast-followup-chips"><button type="button" data-follow-days="1">Tomorrow</button><button type="button" data-follow-days="3">+3 days</button><button type="button" data-follow-days="7">+7 days</button><button type="button" data-follow-days="0">Clear</button></div>
        ...
        <div class="sticky-save"><button id="meetingSaveBtn" type="submit" class="btn primary full">Save meeting + 1 call</button></div>
      </form>`);
  ...
  function editRecord(type,id='') {
    ...
    openSheet(`${id?'Edit':'Add'} ${isDoctor?'doctor':'chemist'}`,isDoctor?'Doctor name, hospital/clinic, address and linked chemist are saved once.':'Only shop name and location are needed.',`
      <form id="recordForm" class="sheet-form">
        ...
        ${isDoctor?' </div></details>':''}
        <div class="button-row sticky-fast-save">${id?`<button type="button" class="btn danger" id="deleteRecordBtn">Delete</button>`:''}<button type="submit" class="btn primary">${isDoctor?'Save doctor':'Save once'}</button></div>
      </form>`);
    if(isDoctor){
      const nameInput=$('#recordName'),hospitalInput=$('#recordHospital'),areaInput=$('#recordForm input[name="area"]'),googleBtn=$('#recordGoogleVerifyBtn'),googleQuery=$('#recordGoogleVerifyQuery');
    ...
    form.addEventListener('submit',e=>{
      ...
      rec.name=clean(fd.get('name'));rec.address=clean(fd.get('address'));rec.area=clean(fd.get('area'));rec.hq=rec.hq||state.profile.hq;rec.notes=clean(fd.get('notes'));if(isDoctor){rec.hospital=clean(fd.get('hospital'));if(!rec.hospital){toast('Hospital / clinic / firm name is required.');return;}if(!id&&state.doctors.some(d=>norm(d.name)===norm(rec.name)&&norm(doctorHospital(d))===norm(rec.hospital))){toast('This doctor + hospital already exists. Open the existing doctor instead.');return;}rec.area=rec.area||inferDoctorArea(rec)||rec.hq;rec.needsCompletion=!(rec.name&&norm(rec.name)!==norm(rec.hospital));}
